...
- **Removed all skill vendoring from the npm package**: The npm package now ships ONLY the CLI (`dist/`), platform-specific agents/instructions/prompts (`.github/`), and `AGENTS.template.md`. NO skills are shipped in the npm package — zero vendoring.
- **Our 9 custom skills pulled via `npx skills add kylebrodeur/wordpress-agent-kit`**: `wp-bootstrap`, `wp-gravity-connect`, `wp-gravity-forms`, `wp-gravity-smtp`, `wp-gravity-stack`, `wp-gravityview`, `wp-gravity-wiz`, `wp-pods`, `wp-wpengine` live in the top-level `skills/` directory, which is committed to git (the marketplace source for `npx skills`) but EXCLUDED from the npm package (`.npmignore`). They are no longer bundled/vendored into the npm package.
- **17 upstream skills pulled via `npx skills add WordPress/agent-skills`**: `blueprint`, `wordpress-router`, `wp-abilities-api`, `wp-abilities-audit`, `wp-abilities-verify`, `wp-block-development`, `wp-block-themes`, `wp-interactivity-api`, `wp-performance`, `wp-phpstan`, `wp-playground`, `wp-plugin-development`, `wp-plugin-directory-guidelines`, `wp-project-triage`, `wp-rest-api`, `wp-wpcli-and-ops`, `wpds` come from the WordPress/agent-skills GitHub repo via `npx skills`. We do NOT maintain or vendor them.
- **`skills install` runs two `npx skills add` commands**: both target the universal `.agents/skills/` directory ONLY (via `--agent cursor`, which maps to the shared `.agents/skills/` dir — no per-agent directories like `.claude/skills` or `.cursor/skills`): (1) `npx skills add kylebrodeur/wordpress-agent-kit --agent cursor --yes` (our 9), (2) `npx skills add WordPress/agent-skills --agent cursor --yes` (the 17 upstream). Result: all 26 skills land in `<target>/.agents/skills/` as real files plus a `skills-lock.json` tracking both sources. Requires network + npx.
- **`skills update` = `npx skills update --yes`**: refreshes all installed skills in `.agents/skills/`.
- **`install` no longer touches skills**: `wp-agent-kit install` installs platform agents/instructions/prompts + the `AGENTS.md` template only, and prints a hint to run `wp-agent-kit skills install`.
- **`.agents/skills/` is dev-only (gitignored, generated by `skills install`)**: NOT committed, NOT shipped.
- **`skills/` IS committed to git** (marketplace source for our 9) but NOT shipped in the npm package.
- **Simplified skill flags**: `skills install` and `skills update` accept only `[dir]` (defaults to cwd) and `--dry-run`. Removed `--agent`/`--project-dir`/`--global`/`--force` for simplicity. Users who want agent-specific control can run `npx skills add` directly.

### Removed

- **`sync-skills` command and `syncSkillsApi`**: GONE (removed). Use `wp-agent-kit skills install` / `installSkillsApi` and `wp-agent-kit skills update` / `updateSkillsApi` instead.

### Pi Extension

- **`wp_skills_install` / `wp_skills_update` tools** (with `targetDir` + `dryRun` params only), exposed as `/wp-skills-install` and `/wp-skills-update` commands. Replaces the prior `wp_sync_skills` / `/wp-sync-skills` tooling.
